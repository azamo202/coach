...
### فحص الخادم بنداء واحد

قبل كل رفع، اسأل الخادم عن جاهزيته بنفسه بدل الاعتماد على الذاكرة:

```bash
curl -s https://api.coachmin.tech/subscriptions/diagnostics -H "authorization: Bearer <TOKEN>"
```

`ready: true` وحدها كافية. أما `ready: false` فمعناها أن الرفع بلا معنى،
والحقول تقول ما ينقص بالضبط:

| الحقل | معناه إن كان `false` |
|---|---|
| `storeApi.keyId` / `issuerId` / `bundleId` / `privateKey` | مفتاح App Store Server API ناقص. **كل شراء سيفشل تحقّقه**: المراجع يدفع في Sandbox ولا يحصل على شيء — رفض مؤكّد بإرشاد 3.1.2 |
| `appleRootCertificate` | شهادة جذر Apple غير مثبّتة. شغّل `npm run fetch:apple-root` — إشعارات التجديد والإلغاء كلها ستُرفض بدونها |
| `ai.configured` | `OPENAI_API_KEY` ناقص. التوليد والاستشارة يرجعان 503 صريحاً (وهو المقصود: أفضل من تسليم قالب ثابت لمن دفع) |

> `ai.mockMode` يجب أن يكون `false` في الإنتاج دائماً. مولّد القوالب
> يعمل فقط بـ`MOCK_AI=true` وخارج `NODE_ENV=production`، وقد أُغلق
> عمداً في الإنتاج مهما كانت البيئة.
