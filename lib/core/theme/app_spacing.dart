...
class Space {
  ...
  // -------------------------------------------------------------------
  // أعرض ما يبلغه عمود المحتوى على الأجهزة العريضة
  // -------------------------------------------------------------------
  //
  // ثلاثة عروض فقط، يختارها **نوع المحتوى** لا الشاشة. كان لكل شاشة عرضها
  // (420، 520، 580، 720، 760، 780، 820، 840، 860)، فيتغيّر عرض العمود عند
  // كل انتقال بين تبويب وآخر على الجهاز اللوحي.

  /// نموذج من حقول وأزرار: تسجيل الدخول، إنشاء الحساب، تعديل البيانات.
  ///
  /// أضيق عمداً — الحقل الطويل يصعب مسحه بالعين، ولا يستفيد من العرض.
  static const double formWidth = 460;

  /// محتوى يُقرأ ويُتصفَّح: الرئيسية، البرنامج، الجلسة، التقدّم، الحساب.
  static const double contentWidth = 840;

  /// شبكة بطاقات متعدّدة الأعمدة — «برامجي» وحدها.
  static const double gridWidth = 1200;
...
class IconSizes {
  ...
  /// المربّع الملوّن الذي يجلس فيه الرمز داخل صفّ أو بطاقة.
  ///
  /// كان كل ملف يختار مقاسه بنفسه (36 و38 و50)، فتغيّر ثقل الصفّ بين شاشة
  /// وأخرى بلا سبب. مقاس واحد يكفي كل مواضعه.
